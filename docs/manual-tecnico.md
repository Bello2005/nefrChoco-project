...
| `finalidad_consulta` | Finalidad de la consulta | [RIPSFinalidadConsultaVersion2](https://vulcano.ihcecol.gov.co/CodeSystem-RIPSFinalidadConsultaVersion2.json) | Ídem; confirmado en el anexo de la Res. 948 (ver nota de consultas abajo) |
...
- **Pendiente:** para consultas, el anexo solo acepta la finalidad y la causa que la tabla de SISPRO marca en una columna "para consultas". Los JSON del IHCE no traen esa columna, así que hoy la lista muestra todos los códigos. Para filtrarla hay que exportar esas dos tablas de SISPRO. La zona de residencia del RIPS usa la tabla `ZonaVersion2` de SISPRO: falta cotejarla con `ColombianResidenceZone` del IHCE.
